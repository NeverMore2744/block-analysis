...
ali_t <- read.table(paste0(dirname, "b5_b6_b7_num_ali.data"), 
    header = T, stringsAsFactors = F);
msr_t <- read.table(paste0(dirname, "b5_b6_b7_num_msr.data"), 
    header = T, stringsAsFactors = F);
tc_t <- read.table(paste0(dirname, "b5_b6_b7_num_tc.data"), 
    header = T, stringsAsFactors = F);
...
print(paste0("AliCloud: Minimum active volumes (%): ", 
      min(ali_active$y) / nrow(ali_attr) * 100));
print(paste0("TencentCloud: Minimum active volumes (%): ", 
      min(tc_active$y) / nrow(tc_attr) * 100));
print(paste0("MSRC: Minimum active volumes (%): ",
      min(msr_active$y) / nrow(msr_attr) * 100));
...
proc <- function(file_name, intv, days, pf) {
  ...
  print(paste0(pf, " larger than 95% trace period: ", 
        nrow(subset(df, activeTimeInMin >= whole_period * 0.95)) / 
        nrow(df) * 100.0, " %"));
...
ali_attr <- read.table("b5_b6_b7_time_ali.data", header = T, stringsAsFactors = F);
msr_attr <- read.table("b5_b6_b7_time_msr.data", header = T, stringsAsFactors = F);
tc_attr <- read.table("b5_b6_b7_time_tc.data", header = T, stringsAsFactors = F);
